...
def _components(
    manuals: list[dict[str, Any]], records: list[dict[str, Any]]
) -> dict[str, str]:
    ...
    for record in records:
        manual_ids = [
            document["manual_id"] for document in record.get("source_documents", [])
        ]
...
def export_records(
    ...
    for row in records:
        manual_ids = [
            document["manual_id"] for document in row.get("source_documents", [])
        ] or [row["manual_id"]]
        provenance = {
